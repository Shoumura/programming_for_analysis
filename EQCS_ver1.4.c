...
//////***** values about this software *****//////
  char software_version[N] = "1.4h" ; char developed_date[N] = "2019/11/27";
//////*****                            *****//////

/// variables for file processing
  FILE *fp,*output,*scaledEQlist,*rearrangedEQlist,*supercellxyz,*spEQfile,*gjf,*cif,*ciflist,*supercellgjf,*supercellEQlist,*cellVDlist,*temp_babel,*energy_sort,*density_sort,*forDOSinp,*forDOSsh,*cytoscapeEQlist,*cytoscapeTSlist,*cytoscapePTlist; // file structure for EQlist sorting
  FILE *sublog,*for_test,*re_output,*re_sublog,*temp_inputfile,*temp_outputfile; // file structure for EQcomposition sorting
  char FILENAME_argument[N], original_EQlist_fname[N], JOBNAME[N], xyzfilename[N], tempfname[N], scaled_EQlist_filename[N], rearranged_EQlist_filename[N], cell_VDlist_filename[N], scaled_cell_VDlist_filename[N], cell_dlist_filename[N], dlogfilename[N], elogfilename[N], spfilename[N], gjffilename[N], ciffilename[N], ciflistfilename[N], forDOSinpfilename[N], forDOSshfilename[N], cytofilename[N]; // file name
  ...
  if( argc > 2 ) { // options
    while ( temp_option_num < argc ){
      if( strcmp( argv[temp_option_num] , "-c" ) == 0 || strcmp( argv[temp_option_num] , "-C" ) == 0 ){
        ...
        gjfgen_option = 1;
      }else if( strcmp( argv[temp_option_num] , "-cif" ) == 0 || strcmp( argv[temp_option_num] , "-Cif" ) == 0 || strcmp( argv[temp_option_num] , "-CIF" ) == 0 ){ // added 9613
        cifgen_option = 1;
...
//////*****  initial check for composition
    while( fgets(str, N, fp) != NULL ){
      if( atomcounting < 2 ){
        if( (strstr(str, "#" ) != NULL || strstr(str, "INITIAL" ) != NULL) && atomcounting == 0 ){
          atomcounting = 1;
        }
        if( strstr(str, "#" ) == NULL && strstr(str, "=" ) != NULL && atomcounting == 1 ){
          atomcounting = 2;
          TVcounting = 1;
        ...
        if( strstr(str, "# STEP 1" ) != NULL && JOBtype == 17 && atomcounting == 1 ){
          atomcounting = 2;
          TVcounting = 1;
  ...
  // print date (END)
    ...
    fprintf(output,"                                            produced by H.N. (%s)\n\n",developed_date);
      ...
      while(fgets(str, N, fp) != NULL){
        ...
        if(strstr( str , "Geometry of AppTS" ) != NULL && IRCing == 0){ //"IRC..." found
          if(LUPITRnum > 0){ // first AppTS appeared (8827added)
            ...
            if(LUPOUTout != NULL) { //if LUPOUTt.log exists (8829refined) (9501revised)
              sprintf(forluptofile, "grep NODE -A%d %s_LUPOUTt.log > %s_forLUP_%s.log > /dev/null 2>&1", atoms, JOBNAME, JOBNAME, datenum); // output forLUP file (8827added)
        ...
        if(strstr( str , "INITIAL STRUCTURE" ) != NULL && IRCing == 0){ //"IRC..." found
          fprintf(output, "\n=========================================================================\n");
          fprintf(output, "\n");
          fprintf(output, " <<IRC profiles of (# AppTS %d)>>\n", AppTSnum);
          fprintf(output, "\n");
          ...
          fclose(small);
              ...
              if(strstr( str , "OPTOPTOPT" ) != NULL){ //"OPT..." found
                ...
                if(strstr( str , "Optimized structure" ) != NULL && OPTing == 1){ //"Optimized structure..." found
                  ...
                  if(FORWARD == 1){
                    fprintf(output, "FORWARD (# AppTS %d) %s", AppTSnum,log_temporary[meta_atoms]);
                  }else{
                    if(BACKWARD == 1){
                      fprintf(output, "BACKWARD (# AppTS %d) %s", AppTSnum,log_temporary[meta_atoms]);
              ...
              if(strstr( str , "Thermochemistry" ) != NULL){ //"Thermochemistry" found
                fprintf(output, "\n");
                  if(FORWARD == 1){
                    fprintf(output, "FORWARD (# AppTS %d)", AppTSnum);
                  }else{
                    if(BACKWARD == 1){
                      fprintf(output, "BACKWARD (# AppTS %d)", AppTSnum);
                    }else if( MINoptimization < 0 ){
                      fprintf(output, "TS (# AppTS %d) %s", AppTSnum,log_temporary[meta_atoms]);
                    }else{
                      fprintf(output, "MINopt (AppEQ-%d)\n", MINoptimization);
              ...
              if(strstr( str , "Energy profile along IRC" ) != NULL){ //Energy profile along IRC
                ...
                fprintf(output2, "-------------------------------------------------------------------------\n");
                fprintf(output2, "FORWARD\t=\t%17.12lf\nTS\t=\t%17.12lf\nBACK\t=\t%17.12lf\n", G_FORval, G_TSval, G_BACKval);
                fprintf(output2, "FORWARD\t-> TS =\t%17.12lf\nBACK\t-> TS =\t%17.12lf\n", 2625.49962*(G_TSval-G_FORval),2625.49962*(G_TSval-G_BACKval));
                fprintf(output2, "=========================================================================\n\n");
                fclose(output2);
    ...
    FILE *NODECONNECTION;
    if( no_babel_mode == 1 ){
      sprintf(command, "%s temptemp_eq -c -n > /dev/null 2>&1", argv[0]);
      system(command);
      ...
      sprintf(command, "%s temptemp_eq -c > /dev/null 2>&1", argv[0]);
      system(command);
      sprintf(tempfilename, "EQCS_temptemp_eq_cLOG.log");
      NODECONNECTION = fopen(tempfilename,"r");
      while(fgets(str, N, NODECONNECTION) != NULL){
...
/////////////////////////////// MAIN SECTION ///////////////////////////////
  ...
  if( no_babel_mode == 0 ){
    ...
    if( one_by_one_mode == 0 ){
      ...
      sprintf(command, "babel -i xyz %s -o can tempEQCS_%s.cansmi > /dev/null 2>&1", xyzfilename, JOBNAME);
      system(command);
      ...
      sprintf(command, "babel -i xyz %s -o inchi EQCS_%s.inchi > /dev/null 2>&1", xyzfilename, JOBNAME);
      system(command);
    // for SMILES
    ...
    // for InChI
      ...
      for(j = 0;j < totalEQnum;j++){
        ...
        fclose(temp_babel);
        sprintf(command, "babel -i xyz %s -o can EQ%d_%s.cansmi > /dev/null 2>&1", tempfname, j, JOBNAME);
        system(command);
        sprintf(command, "babel -i xyz %s -o inchi EQ%d_%s.inchi > /dev/null 2>&1", tempfname, j, JOBNAME);
        system(command);
        remove(tempfname);
...
/*  ////////////// element counting section <<begin>>
  printf("\r  runing... (element counting)      ");
    sprintf(totalelementname[0],"%sX",atomname_TVexcluded[0]); ///// !!! element names of "totalelementname" array is "X " <! one space added !>
    totalelementnamenum++;
    for(j = 1;j < atoms;j++){ /// ((for all atoms))
...
//            fprintf(output, "# EQ %d (Old EQ %d) Rel.E=%.1f; Rel.G(0K)=%.1f; Rel.G(RT)=%.1f; (%.3f%%)\n", tempEQnum, oldEQnum[tempEQnum], Electron_Energy[tempEQnum], zero_Gibbs_Energy[tempEQnum], RT_Gibbs_Energy[tempEQnum], Boltzmann_Distribution[tempEQnum]);
      ...
      fprintf(output,"\n=========================================================================\nlist of TS connected to the same composition type\n");
      for(j = 0;j < totalTSTSnum;j++){
        if( TSTS_same_composition_list[j] == 1 && TSTS_DC_num[j] == 0 ){
          fprintf(output," TS %*d\t: %*d - %*d\t( in type %*d )\n",totalTSTS_digitnum,j,totalEQ_digitnum,TSTS_CONNECTION_EQa_num[j],totalEQ_digitnum,TSTS_CONNECTION_EQb_num[j],totalcomposition_digitnum,TSTS_composition_EQa_num[j]);
        }
      }
      fprintf(output,"=========================================================================\nlist of TS connected to \"??\" or \"DC\"\n");
      for(j = 0;j < totalTSTSnum;j++){
        if( TSTS_DC_num[j] == 1 ){
  ...
  if(fp != NULL) { // if PT_list exists <<*begin*>>
    ...
    if( totalPTTSnum > 0 ){
        ...
        for(k = 0;k < totalPTTSnum;k++){ // "k" runs as a tempPTTSnum
          if( PTTS_composition_EQa_num[k] == j && PTTS_same_composition_list[k] == 0 && PTTS_DC_num[k] == 0 ){
            if( no_babel_mode == 0 ){
              ...
              if( i != 1 ){
                fprintf(output," : E = %17.12lf (dE = %.1lf kJ/mol)\n",PTTS_Bare_Energy[k], 2625.49962 * (PTTS_Bare_Energy[k] - Bare_Energy[PTTS_CONNECTION_EQa_num[k]]));
              }
            }else{
              fprintf(output," PT %*d\t: %*d - %*d\t( -> type %*d ) : E = %17.12lf (dE = %.1lf kJ/mol)\n", totalPTTS_digitnum, k, totalEQ_digitnum, PTTS_CONNECTION_EQa_num[k], totalEQ_digitnum, PTTS_CONNECTION_EQb_num[k], totalcomposition_digitnum, PTTS_composition_EQb_num[k], totalcomposition_digitnum, PTTS_composition_EQb_num[k], PTTS_Bare_Energy[k], 2625.49962 * (PTTS_Bare_Energy[k] - Bare_Energy[PTTS_CONNECTION_EQa_num[k]]));
            }
            ...
            if( no_babel_mode == 0 ){
              ...
              if( i != 1 ){
                fprintf(output," : E = %17.12lf (dE = %.1lf kJ/mol)\n", PTTS_Bare_Energy[k], 2625.49962 * (PTTS_Bare_Energy[k] - Bare_Energy[PTTS_CONNECTION_EQb_num[k]]));
              }
            }else{
              fprintf(output," PT %*d\t: %*d - %*d\t( -> type %*d ) : E = %17.12lf (dE = %.1lf kJ/mol)\n", totalPTTS_digitnum, k, totalEQ_digitnum, PTTS_CONNECTION_EQb_num[k], totalEQ_digitnum, PTTS_CONNECTION_EQa_num[k], totalcomposition_digitnum, PTTS_composition_EQa_num[k], PTTS_Bare_Energy[k], 2625.49962 * (PTTS_Bare_Energy[k] - Bare_Energy[PTTS_CONNECTION_EQb_num[k]]));
            }
          }
          i = 0;
        }
      ...
      fprintf(output,"\n=========================================================================\nlist of PT connected to the same composition type\n");
      for(j = 0;j < totalPTTSnum;j++){
        if( PTTS_same_composition_list[j] == 1 && PTTS_DC_num[j] == 0 ){
          fprintf(output," PT %*d\t: %*d - %*d\t( in type %*d )\n",totalPTTS_digitnum,j,totalEQ_digitnum,PTTS_CONNECTION_EQa_num[j],totalEQ_digitnum,PTTS_CONNECTION_EQb_num[j],totalcomposition_digitnum,PTTS_composition_EQa_num[j]);
        }
      }
      fprintf(output,"=========================================================================\nlist of PT connected to \"??\" or \"DC\"\n");
      for(j = 0;j < totalPTTSnum;j++){
        if( PTTS_DC_num[j] == 1 ){
